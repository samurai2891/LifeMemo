...
///
/// Used **only for the UI path** — drives the audio level meter and speech indicator.
/// The speech recognizer receives raw (unprocessed) buffers directly.
/// Samples are never zeroed; `isSpeech` is advisory only.
final class AudioPreprocessor: @unchecked Sendable {
    struct Metrics: Sendable {
        let invocationCount: Int
        let totalInputSampleCount: Int
    }
    ...
    /// Number of consecutive non-speech frames before declaring silence.
    private let hangoverLimit: Int
    private let lock = NSLock()
    private var hangoverCounter: Int = 0
    private var invocationCount: Int = 0
    private var totalInputSampleCount: Int = 0
    ...
    /// Samples are always preserved (never zeroed). `isSpeech` is advisory for UI.
    nonisolated func process(
        _ samples: [Float], sampleRate: Double
    ) -> PreprocessedAudio {
        lock.lock()
        invocationCount += 1
        totalInputSampleCount += samples.count
        lock.unlock()
        ...
        // 1. Noise reduction (high-pass filter only)
        ...
        // 3. Voice activity detection (advisory only)
        let rawSpeech = voiceDetector.detectSpeech(
            amplified, sampleRate: sampleRate
        )
        let isSpeech = applyHangover(rawSpeech)

        // 4. Calculate audio level for UI (always from amplified samples)
        let level = levelMonitor.calculateLevel(
            from: amplified, isSpeech: isSpeech
        )

        // Samples are always preserved — never zeroed
        return PreprocessedAudio(
            samples: amplified, level: level, isSpeech: isSpeech
        )
    }
    ...
    nonisolated func metricsSnapshot() -> Metrics {
        lock.lock()
        let snapshot = Metrics(
            invocationCount: invocationCount,
            totalInputSampleCount: totalInputSampleCount
        )
        lock.unlock()
        return snapshot
    }
    ...
    /// preventing the speech indicator from flickering during brief pauses.
