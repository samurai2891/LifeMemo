...
/// far from the microphone are audible. Uses per-frame RMS calculation
/// with gain limiting to prevent clipping.
struct AutomaticGainController: AudioProcessingStage {
    ...
    let targetRMS: Float
    /// Maximum allowable gain multiplier.
    let maxGain: Float
    ...
    nonisolated func process(_ samples: [Float], sampleRate: Double) -> [Float] {
        ...
        guard overallRMS > 0.0001 else { return samples }
